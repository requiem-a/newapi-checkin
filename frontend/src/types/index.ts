...
// ─────────────────────────────────────────────────────────────────────────
// 健康态：只在前端推导，后端没有任何对应字段
// ─────────────────────────────────────────────────────────────────────────

/**
 * 站点 / 账号存活状态，由一次余额查询的结果推导（见 shared/lib/site-health.ts）。
 * 刻意不放进 QueryResult：它是展示层的推导结论，不是网络上的形状。
 */
export type SiteHealth = "healthy" | "warning" | "error" | "unknown";
